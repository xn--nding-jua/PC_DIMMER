...
uses
  ...
  MMSystem, Circbuf, MidiType, MidiDefs, Delphmcb, Dialogs;
...
procedure ThrowError(ErrorString:string);
begin
//  ShowMessage(ErrorString);
  //raise EmidioutputError.Create(ErrorString);
end;
...
begin
  if FState = mosOpen then
    ThrowError('Change to DeviceID while device was open')
  else
    if (DeviceID >= Integer(midioutGetNumDevs)) and
      (DeviceID <> Integer(MIDI_MAPPER)) then
//      ThrowError('Invalid device ID')
   ...
   { Set the name and other midioutCAPS properties to match the ID }
      ...
      if Ferror > 0 then
        ThrowError(midioutErrorString(FError));
...
begin
  if FState = mosOpen then
    ThrowError('Change to '+NewProductName+' while device was open')
  else
  ...
  machine. The FProductname property will still be set by SetDeviceID }
    ...
    { Loop uses -1 to test for MIDI_MAPPER as well }
      ...
      if FProductName <> NewProductName then
        ThrowError('MIDI output Device '+NewProductName+' not installed')
...
begin
  if FState = mosOpen then
    ThrowError('Change to Product Technology while device was open')
  else
  begin
       { Loop uses -1 to test for MIDI_MAPPER as well }
    for testDeviceID := -1 to (midiOutGetNumDevs - 1) do
    ...
    if FTechnology <> NewTechnology then
      ThrowError('MIDI output technology '+TechName[NewTechnology]+' not installed')
    else
      SetDeviceID(testDeviceID);
  end;
end;
...
begin
  if FState <> mosOpen then
    ThrowError('MIDI Output device not open');
