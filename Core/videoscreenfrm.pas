...
uses
  ...
  ToolWin, gnugettext, TB2Item, TB2Dock, TB2Toolbar;
...
type
  Tvideoscreenform = class(TForm)
    ...
    QuadView: TTBItem;
    N3: TTBSubmenuItem;
    Logfileeinsehen1: TTBItem;
    WasbringtderVideoscreen1: TTBItem;
...
begin
  ...
  begin
    (FilterGraph1 as IMediaSeeking).GetCurrentPosition(CurrentPos);
    (FilterGraph1 as IMediaSeeking).GetStopPosition(StopPos);
    Timelabel1.Caption:=mainform.MillisecondsToTime(RefTimeToMiliSec(CurrentPos))+' ('+inttostr(trunc((CurrentPos/StopPos)*100))+'%)';
  ...
  begin
    (FilterGraph2 as IMediaSeeking).GetCurrentPosition(CurrentPos);
    (FilterGraph2 as IMediaSeeking).GetStopPosition(StopPos);
    Timelabel2.Caption:=mainform.MillisecondsToTime(RefTimeToMiliSec(CurrentPos))+' ('+inttostr(trunc((CurrentPos/StopPos)*100))+'%)';
  ...
  begin
    (FilterGraph3 as IMediaSeeking).GetCurrentPosition(CurrentPos);
    (FilterGraph3 as IMediaSeeking).GetStopPosition(StopPos);
    Timelabel3.Caption:=mainform.MillisecondsToTime(RefTimeToMiliSec(CurrentPos))+' ('+inttostr(trunc((CurrentPos/StopPos)*100))+'%)';
  ...
  begin
    (FilterGraph4 as IMediaSeeking).GetCurrentPosition(CurrentPos);
    (FilterGraph4 as IMediaSeeking).GetStopPosition(StopPos);
    Timelabel4.Caption:=mainform.MillisecondsToTime(RefTimeToMiliSec(CurrentPos))+' ('+inttostr(trunc((CurrentPos/StopPos)*100))+'%)';
...
begin
  ShowMessage(_('Für die Programmierung einer Lichtshow für eine bereits feststehende choreografische Darstellung kann der Videoscreen dazu benutzt werden, eine vorhandene Videoaufzeichnung des Tanzes zeitgleich mit dem Audioeffektplayer ablaufen zu lassen.')+#13#10#13#10+_('Über die Befehle kann der Videoscreen direkt aus der Timeline des Audioeffektplayers gesteuert werden, sodass das Videobild perfekt auf die Lichtshow abgestimmt werden kann.'));
end;
...
procedure Tvideoscreenform.CreateParams(var Params:TCreateParams);
begin
