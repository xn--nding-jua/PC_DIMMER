...
uses
  ...
  Dialogs, StdCtrls, Action, gnugettext;
...
begin
  ...
  for i:=1 to mainform.lastchan do action.ch[i]:=false;
  while pos(',',s)<>0 do {Suche nach Kanälen}
    begin
      cnt:=strtoint(copy(s,pos(',',s)-4,4));
      action.ch[cnt]:=true;
      if cnt<10 then delete(s,pos(inttostr(cnt),s),2)
      else
      if cnt<100 then delete(s,pos(inttostr(cnt),s),3)
      else
      if cnt<1000 then delete(s,pos(inttostr(cnt),s),4)
      else delete(s,pos(inttostr(cnt),s),5);
    end;
  if copy(s,1,1)='W' then
    begin
      action.ch[strtoint(copy(s,pos('for',s)-5,4))]:=true;
  ...
  if copy(s,1,1)='S' then
    begin
      action.ch[strtoint(copy(s,pos('to',s)-5,4))]:=true;
  ...
  if copy(s,1,1)='F' then
    begin
      action.ch[strtoint(copy(s,pos('from',s)-5,4))]:=true;
...
procedure Teditskriptfrm.FormCreate(Sender: TObject);
begin
  TranslateComponent(self);
end;

procedure Teditskriptfrm.CreateParams(var Params:TCreateParams);
begin
