...
BEGIN_MESSAGE_MAP(CDMXDemoDlg, CDialog)
	...
	ON_WM_TIMER()
	//}}AFX_MSG_MAP
	ON_COMMAND(ID_UNIVERSE1, OnUniverse1)
	ON_COMMAND(ID_UNIVERSE2, OnUniverse2)
	ON_COMMAND(ID_UNIVERSE3, OnUniverse3)
	ON_COMMAND(ID_UNIVERSE4, OnUniverse4)
END_MESSAGE_MAP()
...
BOOL CDMXDemoDlg::OnInitDialog()
{
	...
	// Build the DeviceList of DMX4ALL-PC-Interface
	DWORD	numDevs;
	Dmx4allCreateInterfaceList(&numDevs);

	// Connect to the DMX4ALL-Interfaces
	for(DWORD i=0;i<numDevs;i++)
	{
		Dmx4allSetUniverse(i);
		DWORD	PortIndex;
		Dmx4allGetInterfaceDetail(i,&PortIndex,NULL,0);
		Dmx4allOpenPort(PortIndex);

		if(i==0) GetMenu()->EnableMenuItem(ID_UNIVERSE1,MF_ENABLED | MF_BYCOMMAND);
		if(i==1) GetMenu()->EnableMenuItem(ID_UNIVERSE2,MF_ENABLED | MF_BYCOMMAND);
		if(i==2) GetMenu()->EnableMenuItem(ID_UNIVERSE3,MF_ENABLED | MF_BYCOMMAND);
		if(i==3) GetMenu()->EnableMenuItem(ID_UNIVERSE4,MF_ENABLED | MF_BYCOMMAND);
	}

	if(numDevs>0)
	{
		Dmx4allSetUniverse(0);
		GetMenu()->CheckMenuItem(ID_UNIVERSE1,MF_CHECKED | MF_BYCOMMAND);
	}
...
void CDMXDemoDlg::OnUniverse1()
{
	Dmx4allSetUniverse(0);
	GetMenu()->CheckMenuItem(ID_UNIVERSE1,MF_CHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE2,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE3,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE4,MF_UNCHECKED | MF_BYCOMMAND);
}

void CDMXDemoDlg::OnUniverse2()
{
	Dmx4allSetUniverse(1);
	GetMenu()->CheckMenuItem(ID_UNIVERSE1,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE2,MF_CHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE3,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE4,MF_UNCHECKED | MF_BYCOMMAND);
}

void CDMXDemoDlg::OnUniverse3()
{
	Dmx4allSetUniverse(2);
	GetMenu()->CheckMenuItem(ID_UNIVERSE1,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE2,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE3,MF_CHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE4,MF_UNCHECKED | MF_BYCOMMAND);
}

void CDMXDemoDlg::OnUniverse4()
{
	Dmx4allSetUniverse(3);
	GetMenu()->CheckMenuItem(ID_UNIVERSE1,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE2,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE3,MF_UNCHECKED | MF_BYCOMMAND);
	GetMenu()->CheckMenuItem(ID_UNIVERSE4,MF_CHECKED | MF_BYCOMMAND);
}
