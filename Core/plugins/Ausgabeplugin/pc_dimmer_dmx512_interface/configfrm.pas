...
uses
  ...
  Mask, JvExMask, JvSpin, Buttons, PngBitBtn;
...
begin
  ...
  for i:=0 to 31 do
  begin
    if PCD_Interfaces[i].Serial<>'0000000000000000' then
    begin
...
begin
  ...
  begin
    ...
    if newinterface and (PCD_Interfaces[i].Serial<>'0000000000000000') then
    begin
  ...
  for i:=0 to 31 do
  begin
    if (PCD_Interfaces[i].Serial<>'0000000000000000') and (PCD_Interfaces[i].Active) then
    begin
...
begin
  ...
  begin
    ...
    begin
      CloseLink(SerialstringToSerial(Stringgrid1.Cells[1,Stringgrid1.Row]));
      PCD_Interfaces[GetPositionInInterfaceArray(StringGrid1.Cells[1,StringGrid1.Row])].Active:=false;
    end else
    begin
      OpenLink(SerialstringToSerial(Stringgrid1.Cells[1,Stringgrid1.Row]),@DMXOutArray[GetPositionInInterfaceArray(Stringgrid1.Cells[1,Stringgrid1.Row])]);
      PCD_Interfaces[GetPositionInInterfaceArray(StringGrid1.Cells[1,StringGrid1.Row])].Active:=true;

      // Noch alle Werte in der Hardware manuell aktualisieren
      SetUniverse(SerialstringToSerial(Stringgrid1.Cells[1,Stringgrid1.Row]),@DMXOutArray[GetPositionInInterfaceArray(Stringgrid1.Cells[1,Stringgrid1.Row])]);
    end;
    RefreshList;
  end;
  RefreshList;
...
var
  ...
  count, i:integer;
begin
  DLL:=LoadLibrary(PChar(ExtractFilePath(paramstr(0))+'\pcd_dmx512.dll'));
