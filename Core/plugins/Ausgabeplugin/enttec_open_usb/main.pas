...
begin
    ...
    CheckFTDIStatus(FT_SetDataCharacteristics(device_handle, FT_DATA_BITS_8, FT_STOP_BITS_2, FT_PARITY_NONE));
...
begin
  ...
  if oldone<deviceselectionlist.Items.Count then
	...
	begin
		memo1.lines.add('  -> 0 FTDI-Devices found');
	end	else
	begin
		memo1.lines.add('  -> '+inttostr(Num_Devices)+' FTDI-Devices found');
