...
begin
  Result := PChar('Monacor DMX60/DMX120');
end;
...
begin
{
  ...
  begin
    ...
    if address<64 then
      config.dmxout60[address-1]:=endvalue;
    if address<120 then
      config.dmxout120[address-1]:=endvalue;
  end;
}
end;
  ...
  case MSG of
    ...
    begin
      ...
      begin
        ...
        if Data1<64 then
          config.dmxout60[Integer(Data1)-1]:=Data2;
        if Data1<120 then
          config.dmxout120[Integer(Data1)-1]:=Data2;
      end;
    end;
  end;
end;
