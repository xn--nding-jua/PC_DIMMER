...
begin
  ...
  if interface_open > 0 then
   v := DasHardCommand(DHC_DMXOUTOFF, 0, nil)
  else
  begin
  ...
  for i := 0 to 511
    do dmx_level[i] := #0; // CD - Manipulation des "char" pas aussi simple qu'en C
...
begin
  ...
  if interface_open > 0
    then interface_open := DasHardCommand(DHC_CLOSE, 0, nil);
end;
...
// CD - Envoi du block dmx_level à l'interface
    v := DasHardCommand(DHC_DMXOUT, 512, dmx_level);
    if v < 0 then
    begin
     DasHardCommand(DHC_CLOSE, 0, nil);
     interface_open := DasHardCommand(DHC_OPEN, 0, nil);
    end;
  end; {if interface_open then}
end;
...
begin
 ...
 ok := DasHardCommand(DHC_PORTCONFIG, conf, nil);
...
var
 ok : integer;
begin
 ok := DasHardCommand(DHC_RESET, 0, nil);
 StatusBar1.Simpletext := 'Status pour Reset : ' + inttostr(ok);
end;
