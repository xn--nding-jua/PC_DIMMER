...
uses
  ...
  Dialogs, bass, basscd, CHLed, Registry, StdCtrls, Buttons, PngBitBtn,
...
begin
  ...
  shutdown:=false;
	...
	if (HIWORD(BASS_GetVersion) <> BASSVERSION) then
		MessageBox(0,'Die Datei "bass.dll" liegt in einer falschen Version vor.',nil,MB_ICONERROR);
...
begin
  ...
  begin
    ...
    lblStatus.Caption:='Laufwerk entriegelt';
  end
  else
  begin
    BASS_CD_Door(curdrive,BASS_CD_DOOR_LOCK);
    lblStatus.Caption:='Laufwerk verriegelt';
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      label2.Caption:='Keine CD eingelegt...';
    end;
...
begin
{
  ...
  if (isopen) then
    lblStatus.Caption := 'Laufwerk geöffnet '
  else
    lblStatus.Caption := 'Laufwerk geschlossen ';

  if (islock) then
  begin
    lblStatus.Caption := lblStatus.Caption + 'und verriegelt';
    cdopen.Enabled:=false;
  end else
  begin
    lblStatus.Caption := lblStatus.Caption + 'aber entriegelt';
    cdopen.Enabled:=true;
  end;
